...
export default function SyncSettings() {
  ...
  return (
    <div className="space-y-6">
      ...
      {(status === 'downloading' || status === 'loading' || status === 'syncing_images') && (
        <div>
          <div className="flex justify-between text-sm text-gray-400 mb-1">
            <span>
              {status === 'downloading' && 'Downloading database...'}
              {status === 'loading' && 'Loading database...'}
              {status === 'syncing_images' && 'Syncing showcase images...'}
            </span>
            <span>{progress}%</span>
          </div>
